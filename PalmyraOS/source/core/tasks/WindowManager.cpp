...
MouseEvent PalmyraOS::kernel::Window::popMouseEvent()
{
	...
	if (mouseEvents_.empty())
		return {
			.x = static_cast<int>(x - x_),
			.y = static_cast<int>(y - y_),
			.isLeftDown = WindowManager::isLeftButtonDown()
		};
...
std::pair<uint32_t, uint32_t> PalmyraOS::kernel::Window::getPosition()
{
	return { x_, y_ };
}
std::pair<uint32_t, uint32_t> PalmyraOS::kernel::Window::getSize()
{
...
uint32_t PalmyraOS::kernel::WindowManager::getActiveWindowId()
{
	return activeWindowId_;
}

std::pair<int, int> PalmyraOS::kernel::WindowManager::getMousePosition()
{
	return { mouseX_, mouseY_ };
}

bool PalmyraOS::kernel::WindowManager::isLeftButtonDown()
{
	return isLeftButtonDown_;
}
