...
int PalmyraOS::kernel::loadElf(PalmyraOS::kernel::KVector<uint8_t>& elfFileContent) {
    ...
    for (int i = 0; i < elfHeader->e_phnum; ++i) {
        cout << DEC() << i << HEX() << ", Type: " << programHeader[i].p_type << ", Offset: " << programHeader[i].p_offset
             << ", VAddr: " << programHeader[i].p_vaddr << ", PAddr: " << programHeader[i].p_paddr << ", FileSize: " << programHeader[i].p_filesz
             << ", MemSize: " << programHeader[i].p_memsz << ", Flags: " << programHeader[i].p_flags << ", Align: " << programHeader[i].p_align << "\n"
             << SWAP_BUFF();
    }
    ...
    const char* shstrtab       = reinterpret_cast<const char*>(elfFileContent.data() + sectionHeaders[elfHeader->e_shstrndx].sh_offset);
    ...
    for (int i = 0; i < elfHeader->e_shnum; ++i) {
        ...
        cout << DEC() << i << HEX() << ", Name: " << sectionName << ", Type: " << sectionHeaders[i].sh_type << ", Flags: " << sectionHeaders[i].sh_flags
             << ", Addr: " << sectionHeaders[i].sh_addr << ", Offset: " << sectionHeaders[i].sh_offset << ", Size: " << sectionHeaders[i].sh_size
             << ", Link: " << sectionHeaders[i].sh_link << ", Info: " << sectionHeaders[i].sh_info << ", Addralign: " << sectionHeaders[i].sh_addralign
             << ", Entsize: " << sectionHeaders[i].sh_entsize << "\n"
             << SWAP_BUFF();
    }
