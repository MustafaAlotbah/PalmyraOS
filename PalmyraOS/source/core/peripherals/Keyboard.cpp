...
const char qwertzToAscii[128] = {0,                                                  // Null
                                 27,                                                 // Escape
                                 ...
                                 '\n',  // Enter = \n
                                 0,     // ctrl
                                 'a',  's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'o', 'a', '#', 0, '#', 'y', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/',
                                 0,  // right shift
                                 '*',  0,   ' '};
...
bool PalmyraOS::kernel::Keyboard::togglKey(PalmyraOS::kernel::Keyboard::LockKey lockKey) { return false; }
...
void PalmyraOS::kernel::Keyboard::toggleLockKeys(uint8_t keyCode) {
    ...
    if (condition_01 || condition_02 || condition_03) { dataPort_.write(0x0); }
    else { dataPort_.write(0x07); }
...
uint32_t* PalmyraOS::kernel::Keyboard::handleInterrupt(PalmyraOS::kernel::interrupts::CPURegisters* regs) {
    ...
    else if (keyIndex == 0x3A || keyIndex == 0x45 || keyIndex == 0x46) {  // CTRL
    ...
    if (/*state == KeyState::RELEASED && */ buffer_[buffer_last_index_] != 0) {
        WindowManager::queueKeyboardEvent({buffer_[buffer_last_index_], state == KeyState::PRESSED, isCtrlPressed_, isShiftPressed_, isAltPressed_, true});
    }
