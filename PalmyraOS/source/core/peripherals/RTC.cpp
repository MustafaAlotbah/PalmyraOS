...
void PalmyraOS::kernel::RTC::initialize() {
    if (!vfs::VirtualFileSystem::isInitialized()) { kernel::kernelPanic("Called '%s' before initialized the VFS", __PRETTY_FUNCTION__); }
...
void PalmyraOS::kernel::RTC::update() {
    ...
    year_     = (bcd_to_dec(kernel::CMOS::read(RTC_Century)) * 100) + bcd_to_dec(kernel::CMOS::read(RTC_Year));
...
bool PalmyraOS::kernel::RTC::isLeapYear(uint32_t year) { return (year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)); }

uint16_t PalmyraOS::kernel::RTC::daysInYear(uint32_t year) { return isLeapYear(year) ? 366 : 365; }

uint32_t PalmyraOS::kernel::RTC::daysSinceEpoch(uint16_t year, uint8_t month, uint8_t day) {
    int days = 0;
    for (int y = epochYear; y < year; ++y) { days += daysInYear(y); }
    int days_in_months[] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    if (isLeapYear(year)) {
        days_in_months[1] = 29;  // February has 29 days in a leap year
    }
    for (int m = 0; m < month - 1; ++m) { days += days_in_months[m]; }
    days += day - 1;  // Subtract 1 because the day is inclusive
    return days;
}

uint64_t PalmyraOS::kernel::RTC::toEpochTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second) {
    uint32_t days    = daysSinceEpoch(year, month, day);
    uint64_t seconds = days * secondsInDay + hour * secondsInHour + minute * secondsInMinute + second;
    return seconds;
}
...
int PalmyraOS::kernel::RTC::ioctl(int request, void* arg) {
    ...
    destination_->tm_yday  = static_cast<int>(daysSinceEpoch(year_, month_, day_) - daysSinceEpoch(year_, 1, 1));
