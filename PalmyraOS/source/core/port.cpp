...
namespace PalmyraOS::kernel::ports
{
  ...
  BasePort::BasePort(uint16_t portNumber)
	  : portNumber_{ portNumber }
  {}

  // Write a byte to a port
  void BytePort::write(uint32_t data) const
  {
	  __asm__ __volatile__("outb %0, %1" : : "a"(static_cast<uint8_t>(data)), "Nd"(portNumber_));
  }
  ...
  void WordPort::write(uint32_t data) const
  {
	  __asm__ __volatile__("outw %0, %1" : : "a"(static_cast<uint16_t>(data)), "Nd"(portNumber_));
  }
  ...
  void DoublePort::write(uint32_t data) const
  {
	  __asm__ __volatile__("outl %0, %1" : : "a"(data), "Nd"(portNumber_));
  }
