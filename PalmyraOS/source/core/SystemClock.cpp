...
void PalmyraOS::kernel::SystemClock::initialize(uint32_t frequency) {
    ...
    if (!setFrequency(frequency)) { kernelPanic("Invalid frequency (%d)", frequency); }
...
uint64_t PalmyraOS::kernel::SystemClock::getMilliseconds() { return (getTicks() * 1000) / frequency_; }

uint64_t PalmyraOS::kernel::SystemClock::getNanoseconds() { return (getTicks() * 1'000'000) / frequency_; }

uint64_t PalmyraOS::kernel::SystemClock::getSeconds() { return getTicks() / frequency_; }
