...
void PalmyraOS::kernel::CPU::initialize() { detectCpuFrequency(); }
...
uint32_t PalmyraOS::kernel::CPU::getL2CacheSize() {
    ...
    return ((result.ebx >> 22) + 1) * ((result.ebx >> 12 & 0x3FF) + 1) * ((result.ebx & 0xFFF) + 1) * (result.ecx + 1) / 1024;
}
...
uint32_t PalmyraOS::kernel::CPU::detectCpuFrequency() {
    ...
    while (secondsOfDayEnd < secondsOfDayStart + secondsMeasurement) { secondsOfDayEnd = RTC::getSecondsOfDay(); }
