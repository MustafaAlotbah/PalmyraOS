...
namespace std {
    ...
    void __throw_bad_optional_access() { PalmyraOS::kernel::kernelPanic("bad_optional_access: attempted to access an empty optional"); }
    ...
    void __throw_bad_function_call() {
        if (badFunctionCallHandler_) badFunctionCallHandler_("Bad Function Call: Attempted to invoke an uninitialized function object");
        else PalmyraOS::kernel::kernelPanic("Bad Function Call: Attempted to invoke an uninitialized function object");
    }
}  // namespace std

void PalmyraOS::kernel::runtime::setLengthErrorHandler(ExceptionHandler handler) { lengthErrorHandler_ = handler; }

void PalmyraOS::kernel::runtime::setOutOfRangeHandler(ExceptionHandler handler) { outOfRangeErrorHandler_ = handler; }

void PalmyraOS::kernel::runtime::setBadFunctionCallHandler(ExceptionHandler handler) { badFunctionCallHandler_ = handler; }

extern "C" void abort(void) { PalmyraOS::kernel::kernelPanic("Abort called!"); }