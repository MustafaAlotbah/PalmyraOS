...
PalmyraOS::kernel::GDT::SegmentDescriptor::SegmentDescriptor(uint32_t address, uint32_t size, uint8_t access_type, uint8_t flags)
    : addr_0_15{(uint16_t) (address & 0xFFFF)}, size_0_15{(uint16_t) (size & 0xFFFF)}, addr_16_23{(uint8_t) ((address >> 16) & 0xFF)}, access_type{access_type},
      flag_size_16_19{(uint8_t) (((size >> 16) & 0x0F) | (flags & 0xF0))}, addr_24_31{(uint8_t) ((address >> 24) & 0xFF)} {}
...
void PalmyraOS::kernel::GDT::GlobalDescriptorTable::setKernelStack(uint32_t esp) { tss_entry.esp0 = esp; }
