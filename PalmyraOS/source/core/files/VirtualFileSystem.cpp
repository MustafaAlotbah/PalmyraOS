...
namespace PalmyraOS::kernel::vfs {
    ...
    FunctionInode::FunctionInode(ReadFunction readFunc, WriteFunction writeFunc, IOCTLFunction ioctlFunc)
        : InodeBase(Type::CharacterDevice, Mode::USER_READ | Mode::USER_WRITE | Mode::GROUP_READ | Mode::OTHERS_READ, UserID::ROOT, GroupID::ROOT),
          readFunction_(std::move(readFunc)), writeFunction_(std::move(writeFunc)), ioctlFunction_(std::move(ioctlFunc)) {}
    ...
    InodeBase* VirtualFileSystem::getInodeByPath(InodeBase& rootInode, const KString& path) { return traversePath(rootInode, path.split('/', true)); }
    ...
    InodeBase* VirtualFileSystem::createDirectory(const KString& path, InodeBase::Mode mode, InodeBase::UserID userId, InodeBase::GroupID groupId) {
        // Ensure the root node is initialized before proceeding
        if (!rootNode_) kernelPanic("Called %s before initializing rootNode_", __PRETTY_FUNCTION__);
        ...
        auto directory = kernel::heapManager.createInstance<InodeBase>(InodeBase::Type::Directory, mode, userId, groupId);
    ...
    InodeBase* VirtualFileSystem::getRootInode() { return rootNode_; }
    ...
    bool VirtualFileSystem::initializeDeviceFileSystem() {
        ...
        deviceInode_ = devSuperBlock->allocateInode(InodeBase::Type::Directory,
        ...
        binaryInode_ = devSuperBlock->allocateInode(InodeBase::Type::Directory,
                                                    Mode::USER_READ | Mode::USER_WRITE | Mode::GROUP_READ | Mode::OTHERS_READ,
                                                    InodeBase::UserID::ROOT,
                                                    InodeBase::GroupID::ROOT);
    ...
    bool VirtualFileSystem::isInitialized() { return rootNode_; }
