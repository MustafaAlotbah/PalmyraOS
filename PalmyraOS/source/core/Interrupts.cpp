...
PalmyraOS::kernel::interrupts::PICManager* PalmyraOS::kernel::interrupts::InterruptController::activePicManager = nullptr;
...
extern "C" uint32_t* primary_isr_handler(PalmyraOS::kernel::interrupts::CPURegisters* registers) {
    ...
    if (!handled) { panicRegisters("Unhandled Interrupt!", registers); }
...
PalmyraOS::kernel::interrupts::InterruptDescriptorTable::InterruptDescriptorTable(PalmyraOS::kernel::GDT::GlobalDescriptorTable* gdt) {
    ...
    for (int i = 0; i < 256; ++i) { setDescriptor(i, codeSegment, &_default_isr_handler, 0, GateType::InterruptGate); }
}

void PalmyraOS::kernel::interrupts::InterruptDescriptorTable::setDescriptor(uint8_t interruptVector,
    ...
    entry.attributes                       = static_cast<uint8_t>(gateType)
                       ...
                       | (privilegeRing << DESCRIPTOR_PRIVILEGE_BIT) | (1 << PRESENT_BIT);
}
...
PalmyraOS::kernel::interrupts::PICManager::PICManager()
    : masterPicCommand{PORT_PIC_MASTER_CMD}, masterPicData{PORT_PIC_MASTER_DATA}, slavePicCommand{PORT_PIC_SLAVE_CMD}, slavePicData{PORT_PIC_SLAVE_DATA} {
...
PalmyraOS::kernel::interrupts::InterruptController::InterruptController(GDT::GlobalDescriptorTable* gdt) : idtHandler(gdt) {
...
void PalmyraOS::kernel::interrupts::InterruptController::disableInterrupts() { disable_interrupts(); }

void PalmyraOS::kernel::interrupts::InterruptController::setInterruptHandler(uint8_t interrupt_number, InterruptHandler interrupt_handler) {
