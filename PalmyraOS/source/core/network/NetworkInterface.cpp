...
namespace PalmyraOS::kernel {
    ...
    void* NetworkInterface::operator new(size_t size) {
    ...
    NetworkInterface::NetworkInterface(const char* name, const uint8_t macAddress[MAC_ADDRESS_SIZE],
                                       types::HeapManagerBase* heapManager)
        : heapManager_(heapManager),
          ipAddress_(0), subnetMask_(0), gateway_(0),
          state_(State::Down), mtu_(STANDARD_MTU),
          promiscuousMode_(false),
          txPackets_(0), rxPackets_(0), txBytes_(0), rxBytes_(0),
          txErrors_(0), rxErrors_(0), rxDropped_(0) {
        
        // Copy interface name (max 15 chars + null terminator)
        memset(name_, 0, sizeof(name_));
        if (name) {
            strncpy(name_, name, MAX_NAME_LENGTH);
        }

        // Copy or zero MAC address
        if (macAddress) {
            memcpy(macAddress_, macAddress, MAC_ADDRESS_SIZE);
        } else {
    ...
    void NetworkInterface::setIPAddress(uint32_t ip) {
        ipAddress_ = ip;
    }

    void NetworkInterface::setSubnetMask(uint32_t mask) {
        subnetMask_ = mask;
    }

    void NetworkInterface::setGateway(uint32_t gateway) {
        gateway_ = gateway;
    }

    void NetworkInterface::setMTU(uint16_t mtu) {
        mtu_ = mtu;
    }
