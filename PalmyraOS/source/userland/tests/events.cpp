...
int PalmyraOS::Userland::Tests::events::main(uint32_t argc, char** argv) {
    ...
    while (true) {
        ...
        windowGui.text() << "Keyboard: "
                         << "key: '" << keyboardEvent.key << "' [" << (keyboardEvent.isCtrlDown ? "CTRL " : "") << (keyboardEvent.isAltDown ? "ALT " : "")
        ...
        windowGui.text() << "Mouse: "
                         << "Coors: (" << mouseEvent.x << ", " << mouseEvent.y << ") [" << (mouseEvent.isLeftDown ? "LEFT " : "")
                         << (mouseEvent.isMiddleDown ? "MIDDLE " : "") << (mouseEvent.isRightDown ? "RIGHT " : "") << "]\n";
        ...
        else { windowGui.text() << "Clicked " << clickCounter << " times.\n"; }

        if (windowGui.link("or click me")) { clickCounter++; }

        windowGui.text() << "\n";

        if (windowGui.link("scroll down", true)) { scrollY--; }

        windowGui.text() << " ";

        if (windowGui.link("scroll up", true)) { scrollY++; }
