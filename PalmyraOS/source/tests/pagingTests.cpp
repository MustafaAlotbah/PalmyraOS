...
void PalmyraOS::Tests::Paging::PagingTester::setup() { kernel::PagingManager::setSecondaryPageFaultHandler(PagingTester::testingPageFaultHandler); }
...
void PalmyraOS::Tests::Paging::PagingTester::testingPageFaultHandler(PalmyraOS::kernel::interrupts::CPURegisters* regs,
...
void PalmyraOS::Tests::Paging::PagingTester::setFaultRecoveryAddress(volatile void* address) { faultRecoveryAddress_ = address; }
/// endregion
...
clean:
    ...
    allocatedPages_1 = kernel::PagingManager::getCurrentPageDirectory()->getNumAllocatedPages();
    if (allocatedPages_1 - allocatedPages > 1) result = false;  // maximum 1 for the new table (which is not de-allocated)
...
bool PalmyraOS::Tests::Heap::testHeapCoalescence() {
...

    {
        ...
        if (freeMemoryAfter > freeMemoryBefore) { result = true; }
    }
