...
//		  if (p == nullptr) kernel::kernelPanic("Heap Allocator Error::construct p=nullptr!");
  ...
  class UVector : public std::vector<T, Alloc>
  {
   ...
   public:
	  explicit UVector(UserHeapManager& heap)
		  : Base(Alloc(heap))
	  {}

	  UVector(UserHeapManager& heap, size_t count)
		  : Base(count, Alloc(heap))
	  {}

	  UVector(UserHeapManager& heap, size_t count, const T& value)
		  : Base(count, value, Alloc(heap))
	  {}

	  template<typename InputIt>
	  UVector(UserHeapManager& heap, InputIt first, InputIt last)
		  : Base(first, last, Alloc(heap))
	  {}

	  UVector(UserHeapManager& heap, std::initializer_list<T> init)
		  : Base(init, Alloc(heap))
	  {}
  };
  ...
  class UString : public types::string<T, HeapAllocator>
  {
   ...
   public:
	  ...
	  explicit UString(UserHeapManager& heap)
	  ...
	  UString(UserHeapManager& heap, size_t count, const T& value)
		  : Base(count, value, HeapAllocator<T>(heap))
	  {}

	  template<typename InputIt>
	  UString(UserHeapManager& heap, InputIt first, InputIt last)
		  : Base(first, last, HeapAllocator<T>(heap))
	  {}
